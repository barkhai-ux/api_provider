...
test("documentation navigation", async ({ page }) => {
  ...
  for (const slug of ["getting-started", "authentication", "routing", "errors", "rate-limits", "versioning", "examples"]) {
...
test("OpenAPI schema is served by the API", async ({ request }) => {
  ...
  expect(Object.keys(schema.paths)).toEqual(expect.arrayContaining(["/v1/geocode", "/v1/route"]));
  expect(Object.keys(schema.paths)).not.toContain("/v1/reverse-geocode");
});
